 ...
 * @author mac
 */
public class ViewYearlySales implements Initializable {
    ...
    public void searchSales(ActionEvent evt) {
        if (mYearSelect.getText().trim().isEmpty()) {
            Init.alertMsg("Missing field", "Please select year");
        } else {
            salesList.clear();
            String year = mYearSelect.getText();
            String[] names = {};
            String[] values = {};
            double totalPrice = 0;
            ResultSet rs = SQLiteConnection.select("SELECT s.id, s.customer_name, s.date, SUM(si.total) as total FROM sales as s LEFT JOIN sales_items as si ON s.id = si.sales_id WHERE s.date LIKE '%" + year + "' GROUP BY si.sales_id ORDER by s.id DESC", names, values);
            try {
                while (rs.next()) {
                ...
                Double credits = getCredits(year);
